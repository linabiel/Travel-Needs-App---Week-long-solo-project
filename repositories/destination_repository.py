...
def select(id):
    ...
    if result is not None:
        destination = Destination(result['user_id'], result['country_id'], result['city_id'], result['visited'], result['id'])
    return destination
...
def update(destination):
    sql = "UPDATE destinations SET (visited, user_id, country_id, city_id) = (%s, %s, %s, %s) WHERE id = %s"
    values = [destination.visited, destination.user.id, destination.country.id, destination.city.id]
    run_sql(sql, values)